const crypto = require('crypto')
const f = require('./functions')

/* ------------------------------ 1. 암복호화 키 생성 --------------------------- */

let sessionId = crypto.randomUUID()
let secretKey = f.generateRandomBytes(32)
let iv = f.generateRandomBytes(12)

/* ------------------------------ 2. 세션키 생성 ------------------------------- */

// base64PublicKey 는 사전에 전달 받은 공개키 입니다.
let base64PublicKey = ''

// API 요청 파라미터에 넣어주세요.
let sessionKey = f.generateSessionKey(sessionId, secretKey, iv, base64PublicKey)
console.log('sessionKey: ' + sessionKey)

/* ------------------------------ 3. 개인정보 암호화 ----------------------------- */

let userName = '김토스'
let encryptedUserName = f.encryptData(sessionId, secretKey, iv, userName) // 암호화된 개인 정보
console.log('encryptedUserName: ' + encryptedUserName)

/* ------------------------------ 4. 개인정보 복호화 ----------------------------- */

// 응답을 받은 경우, 요청을 보낼 때 생성했던 secretKey, iv 를 가지고 있어야 합니다.
// encryptedUserName 이 응답 받은 암호화된 userName 이라고 가정합니다.
let decryptedUserName = f.decryptData(secretKey, iv, encryptedUserName)
console.log('decryptedUserName: ' + decryptedUserName)

/* ------------------------------ 5. 암복호화 결과 검증 --------------------------- */

if (decryptedUserName !== userName) {
    console.error('암복호화 결과가 일치하지 않습니다.')
}